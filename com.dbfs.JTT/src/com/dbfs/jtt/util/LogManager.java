...
public class LogManager extends Thread {
  ...
  private String filePath = "log" + File.separatorChar;
  ...
  public static void logStack(StackTraceElement[] stackElements, String exName) {
    ...
    for (int i = 0, n = stackElements.length; i < n; i++) {
      currentStack += SWT.TAB + stackElements[i].toString() + SWT.LF;
    }
    ...
    if (isDialogs()) {
      ...
      for (int i = 0, n = stackElements.length; i < n; i++) {
        currentStack1 += SWT.TAB + stackElements[i].toString() + SWT.LF;
      }
  ...
  /*** private static boolean isLogging() {
      ...
      return preferences.getBoolean(LoggingPreferencePage.IS_LOGGING, false);
  }*/
