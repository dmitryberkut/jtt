...
public class SOAPSession implements IAdaptable {
    ...
    public void updateWorklog(final Task task, long millisis) throws RemotePermissionException, RemoteValidationException, org.swift.common.soap.jira.RemoteException, RemoteException {
        ...
        for (int i = 0; i < worklogs.length; i++) {
        	...
        	if(tmpCalendar.get(Calendar.DAY_OF_YEAR)==calendar.get(Calendar.DAY_OF_YEAR) 
        			&& tmpCalendar.get(Calendar.YEAR)==calendar.get(Calendar.YEAR)){ // same day
        ...
        try {
            ...
            for (BasicIssue bIssue : searchResult.getIssues()) {
                ...
                if (monitor != null) {
                    ...
                    monitor.subTask("[" + (++i) + "/" + searchResult.getTotal() + "]" + "[" + keyPrj + "][" + bIssue.getKey() + "][" + issue.getSummary() + "]");
                }
